...
async function fetchWithTimeout(
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; RemarkLinkCard/1.0; +https://github.com)",
...
export async function fetchOGP(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[remark-link-card] Failed to fetch ${url}: ${response.status}`
      );
      return metadata;
    }
...
export function generateLinkCardHtml(
 ...
 * CSSやJSで自由にスタイリング可能
 */
export function generateHeadlessHtml(
  metadata: LinkCardMetadata,
  className: string
): string {
  const { url, title, description, image, siteName } = metadata;
...
export function remarkLinkCard(options: RemarkLinkCardOptions = {}) {
  ...
  return async (tree: Root) => {
    ...
    if (urlsToFetch.length > 0) {
      console.log(
        `[remark-link-card] Fetching ${urlsToFetch.length} URL(s)...`
      );

      const results = await Promise.all(
        urlsToFetch.map((url) => fetchOGP(url, opts.fetchTimeout))
      );
